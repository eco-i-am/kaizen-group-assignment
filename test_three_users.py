...
def test_three_users():
    """Test with Morris, Gerard, and John Michael Joseph Pineda to see the conflict"""
    
    print("🧪 TESTING THREE USERS")
    print("=" * 60)
    ...
    # Create a test dataset with these three users
    test_data = [morris_user, gerard_user, john_user]
    ...
    print(f"  John: {get_value(john_user, 'name', '')} (ID: {get_value(john_user, 'user_id', '')})")
    ...
    print(f"    Team Name: {get_value(john_user, 'temporary_team_name', '')}")
    
    # Run grouping on these three users
    ...
    for i, group in enumerate(solo_groups):
        for member in group:
            ...
            if str(user_id).strip() in ['2123', '2360', '2817']:
