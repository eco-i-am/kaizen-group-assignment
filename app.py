...
def show_dashboard():
    ...
    with col1:
        ...
        fig = px.pie(values=gender_counts.values, names=gender_counts.index, 
                    title="Participants by Gender Identity")
...
def show_analysis_page():
    ...
    with tab1:
        ...
        with col2:
            # Experience levels (if available)
    ...
    with tab2:
        ...
        with col2:
            ...
            ph_vs_int = data['residing_in_philippines'].value_counts()
            fig = px.pie(values=ph_vs_int.values, names=['International', 'Philippines'],
                        title="Philippines vs International")
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        with col1:
            # Gender preferences
            gender_pref_counts = data['group_gender_preference'].value_counts()
            fig = px.pie(values=gender_pref_counts.values, names=gender_pref_counts.index,
                        title="Gender Preference Distribution")
            st.plotly_chart(fig, use_container_width=True)
        
        with col2:
            # Solo vs Group preferences
            solo_counts = data['go_solo'].value_counts()
            fig = px.pie(values=solo_counts.values, names=['Group', 'Solo'],
                        title="Solo vs Group Preference")
            st.plotly_chart(fig, use_container_width=True)
