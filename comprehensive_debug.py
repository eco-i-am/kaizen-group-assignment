...
print('Comprehensive debugging of missing users:')
print('=' * 60)
    ...
    if isinstance(accountability_buddies, str):
        ...
        if requested_emails:
            ...
            if request_key not in processed_requests:
                ...
                if participant_email in target_emails:
                    print(f'  ✅ Processing this request')
                ...
                if participant_email in target_emails:
                    print(f'  ❌ Request key already processed, skipping')
        else:
            if participant_email in target_emails:
...
print(f'\nExample groups:')
for i, group in enumerate(requested_groups[:5]):
    group_emails = [get_value(member, 'email', '').lower().strip() for member in group]
    print(f'  Group {i+1}: {group_emails}') 